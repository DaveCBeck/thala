"""
Modified surya recognition module with parallel postprocessing.
...
def _process_single_slice(
    ...
    """
    Process a single slice's tokens into TextChar objects.
    This is the parallelizable unit of work from get_bboxes_text.
    """
    blank_bbox = [[0, 0], [0, 1], [1, 1], [1, 0]]

    if processor.no_output_token in image_tokens:
        return None
...
class RecognitionPredictor(BasePredictor):
    ...
    def get_bboxes_text(
        ...
        """
        Convert predicted tokens to TextChar objects.

        This method uses parallel processing when there are enough slices,
        similar to how detection uses parallel postprocessing.
        """
        needs_boxes = [
        ...
        num_slices = len(flat["slices"])
        max_workers = min(settings.RECOGNITION_POSTPROCESSING_CPU_WORKERS, num_slices)
        parallelize = (
            not settings.IN_STREAMLIT
            and num_slices >= settings.RECOGNITION_MIN_PARALLEL_THRESH
        )
        executor_cls = ThreadPoolExecutor if parallelize else FakeExecutor
        ...
        with executor_cls(max_workers=max_workers) as executor:
            ...
            char_predictions = [future.result() for future in futures]
