#!/usr/bin/env python3
"""
Test script for multi-language book finding.

Uses the multi_lang workflow with books workflow configuration, producing:
- Per-language book recommendations
- Cross-language comparative analysis
- Final integrated synthesis report

Usage:
    python test_multi_lang_books.py "theme" [quality] --languages <langs>
    python test_multi_lang_books.py "theme" test --languages en,es,de
    python test_multi_lang_books.py "theme" quick --languages major
...
Outputs saved to testing/test_data/:
    multilang-books-{lang}-{datetime}.md     Per-language reports
    multilang-books-comparative-{datetime}.md   Cross-language analysis
    multilang-books-final-{datetime}.md         Final integrated synthesis
...
from scripts.workflow_tests.utils import (
...
configure_logging("multi_lang_books")
...
async def translate_to_english(text: str, source_language: str) -> str:
    ...
    prompt = f"""Translate the following {source_language} book recommendations to English.

Preserve:
- All formatting (headers, lists, citations)
- Book titles (translate but keep original title in parentheses)
- Author names (keep original)
- All recommendation categories and structure
...
async def save_markdown_outputs(result: dict, timestamp: str) -> dict[str, str]:
    ...
    if comparative:
        filename = f"multilang-books-comparative-{timestamp}.md"
        filepath = OUTPUT_DIR / filename
        with open(filepath, "w", encoding="utf-8") as f:
            f.write("# Cross-Language Book Recommendations Comparison\n\n")
            f.write(comparative)
    ...
    if synthesis:
        filename = f"multilang-books-final-{timestamp}.md"
        filepath = OUTPUT_DIR / filename
        with open(filepath, "w", encoding="utf-8") as f:
            f.write("# Integrated Multi-Language Book Recommendations\n\n")
            f.write(synthesis)
...
def print_result_summary(result: dict) -> None:
    ...
    print_section_header("MULTI-LANGUAGE BOOK FINDING RESULTS")
    ...
    for lang_result in lang_results:
        ...
        books = lang_result.get("source_count", 0)
        workflows = lang_result.get("workflows_run", [])
        total_books += books
        print(f"  {lang} ({name}): {books} books via {', '.join(workflows)}")

    print(f"\nTotal books found: {total_books}")
...
def parse_args():
    ...
    parser = create_test_parser(
        description="Run multi-language book finding",
        default_topic="Organizational resilience and adaptive leadership",
        topic_help="Theme for book recommendations",
        epilog_examples="""
Examples:
  %(prog)s "systems thinking" quick --languages en,es,de
  %(prog)s "stoic philosophy" standard --languages major
  %(prog)s "climate adaptation" comprehensive --languages en,zh,de,fr
        """,
    )
...
async def main():
    """Run multi-language book finding."""
    ...
    try:
        ...
        logger.exception(f"Error during multi-lang book finding: {e}")
        raise
