...
from workflows.research.web_research.state import SupervisorDecision
...
async def _get_supervisor_decision_structured(
    ...
    """
    decision: SupervisorDecision = await invoke(
        tier=ModelTier.OPUS,
    ...
    if action == "conduct_research":
        ...
        action_data["questions"] = [
            {"question": q, "context": brief.get("topic", "")} for q in decision.research_questions
        ]
    ...
    logger.debug(f"Supervisor (structured): {action}, reasoning: {decision.reasoning[:100]}...")
