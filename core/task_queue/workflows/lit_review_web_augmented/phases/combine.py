...
10. REFERENCE-BODY CONSISTENCY: Every entry in the References/Sources section must be cited at least once in the body text, and every inline citation must have a corresponding entry in References. Before finalizing, verify: (a) no phantom references — if a source appears in the bibliography but is never cited inline, either integrate its content into the relevant body section or remove the entry; (b) no dangling citations — if a [N] or [@KEY] citation appears inline, its bibliography entry must be present. When the web report contains a finding that you haven't yet integrated (e.g., a company milestone, a market figure, a regulatory event), place it in the most thematically appropriate body section rather than silently dropping both the content and its citation.
11. Preserve epistemic hedges and uncertainty qualifications from BOTH sources. If either report flags a claim as uncertain, contested, lacking independent validation, or based on stale comparisons, that hedge MUST appear in the combined report at the point where the claim is stated. Dropping uncertainty qualifications makes the combined report less honest than its inputs — this is never acceptable. If the web report contains a "Remaining Uncertainties" or equivalent section, integrate those hedges inline at the relevant claims rather than collecting them in a separate section.

12. AVOID REDUNDANCY: When a specific statistic, finding, or argument appears in a thematic section AND in the Discussion/Conclusions, the Discussion should cross-reference or build on the earlier appearance rather than restating it verbatim. Repeating the same figure (e.g., a dollar amount, a percentage, a company name with the same data point) more than twice signals a merge artifact, not emphasis.
