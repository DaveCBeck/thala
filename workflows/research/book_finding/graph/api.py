...
async def book_finding(
    ...
    Each category generates book recommendations (1-7 per category depending
    ...
    Args:
        ...
        quality: Quality tier
            - test: 1 recommendation per category, minimal testing
            ...
            - high_quality: 7 recommendations per category, maximum depth
    ...
    try:
        ...
        logger.info(
            f"Book finding workflow completed with status '{status}' ({len(result.get('processed_books', []))} books processed)"
        )
