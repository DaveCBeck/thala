...
class ExpandContextInput(BaseModel):
    """Input schema for expand_context tool."""

    reference: str = Field(
        description="UUID of a store record, 8-character Zotero key, or content snippet for fuzzy match"
    )
    reference_type: Literal["uuid", "zotero_key", "content", "auto"] = Field(
        default="auto",
        description="Type of reference. 'auto' detects based on format.",
    )
    include_zotero: bool = Field(
        default=True,
        description="If record has zotero_key, also fetch Zotero metadata",
    )
    include_history: bool = Field(
        default=False,
        description="Include edit history from who_i_was",
    )
...
class ExpandContextTool(BaseTool):
    ...
    - Need to understand how a belief/preference evolved over time
    - Want to find more about something you vaguely remember

    Accepts UUID (for internal records), Zotero key (8-char string),
    or a content snippet for fuzzy matching.
    """

    name: str = "expand_context"
    description: str = """Get more detail about a specific memory record or source.
...
Accepts UUID, 8-character Zotero key, or content snippet for fuzzy match."""
    ...
    args_schema: type[BaseModel] = ExpandContextInput
    ...
    def _run(
        ...
        raise NotImplementedError("ExpandContextTool only supports async. Use ainvoke().")
    ...
    async def _arun(
        ...
        """
        Expand context for a reference.

        Args:
            reference: UUID, zotero_key, or content snippet
            reference_type: Type hint ("auto" detects format)
            include_zotero: Fetch linked Zotero metadata
            include_history: Include who_i_was history

        Returns:
            ExpandedContext with full record details
        """
        # Auto-detect reference type
        detected_type = self._detect_reference_type(reference, reference_type)
        ...
        if detected_type == "uuid":
            await self._expand_by_uuid(
                reference, result, include_zotero, include_history
            )
        elif detected_type == "zotero_key":
            await self._expand_by_zotero_key(reference, result)
        elif detected_type == "content":
            await self._expand_by_content(
                reference, result, include_zotero, include_history
            )
    ...
    async def _expand_by_uuid(
        ...
        # 2. Check coherence
        if not record:
            record = await self.store_manager.es_stores.coherence.get(record_id)
            if record:
                source_store = "coherence"

        # 3. Check top_of_mind (ChromaDB)
        if not record:
            chroma_result = await self.store_manager.chroma.get(record_id)
            if chroma_result:
        ...
        if record:
            ...
            if include_zotero and zotero_key:
                await self._fetch_zotero(zotero_key, result)

            # Fetch history if requested
            if include_history:
                await self._fetch_history(record_id, result)
    ...
    async def _expand_by_zotero_key(
        ...
        try:
            zotero_item = await self.store_manager.zotero.get(zotero_key)
            if zotero_item:
                result.found = True
    ...
    async def _expand_by_content(
        ...
        try:
            query_embedding = await self.store_manager.embedding.embed(content_snippet)
    ...
    async def _fetch_zotero(self, zotero_key: str, result: ExpandedContext) -> None:
    ...
    async def _fetch_history(self, record_id: UUID, result: ExpandedContext) -> None:
        """Fetch edit history from who_i_was."""
        try:
            history = await self.store_manager.es_stores.who_i_was.get_history(record_id)
