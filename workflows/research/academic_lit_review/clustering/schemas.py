...
class LLMThemeOutput(BaseModel):
    ...
    name: str = Field(
        description="Clear, descriptive theme name suitable as a section heading"
    )
    ...
    sub_themes: list[str] = Field(
        default_factory=list, description="Sub-themes if the cluster is broad"
    )
    relationships: list[str] = Field(
        default_factory=list, description="How this theme relates to other themes"
    )
...
class ClusterAnalysisOutput(BaseModel):
    ...
    timeline: list[str] = Field(
        default_factory=list, description="Key developments chronologically"
    )
    key_debates: list[str] = Field(
        default_factory=list, description="Main debates and positions"
    )
    ...
    outstanding_questions: list[str] = Field(
        default_factory=list, description="Open research questions"
    )
...
class ThematicClusterOutput(BaseModel):
    ...
    key_papers: list[str] = Field(
        default_factory=list, description="Most central papers"
    )
    sub_themes: list[str] = Field(
        default_factory=list, description="Finer-grained topics"
    )
    conflicts: list[str] = Field(
        default_factory=list, description="Contradictory findings"
    )
    gaps: list[str] = Field(default_factory=list, description="Under-researched areas")
    source: str = Field(
        default="merged", description="Origin: bertopic, llm, or merged"
    )
...
class OpusSynthesisOutput(BaseModel):
    ...
    final_clusters: list[ThematicClusterOutput] = Field(
        description="Synthesized thematic clusters"
    )
