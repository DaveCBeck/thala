...
class Settings(BaseSettings):
    ...
    # Parallel postprocessing for recognition (similar to detector)
    RECOGNITION_POSTPROCESSING_CPU_WORKERS: int = min(
        8, os.cpu_count()
    )  # Number of workers for recognition postprocessing
    RECOGNITION_MIN_PARALLEL_THRESH: int = (
        3  # Minimum number of slices before we parallelize
    )
