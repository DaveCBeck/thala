...
async def analyze_for_bases_node(state: Loop2State) -> dict:
    ...
    user_prompt = LOOP2_ANALYZER_USER.format(
        ...
        iteration=iteration + 1,  # Display as 1-indexed for user
...
async def run_mini_review_node(state: Loop2State) -> dict:
    ...
    # Store mini-review data in state for integration
    return {
        "decision": {
            **decision,
...
async def integrate_findings_node(state: Loop2State) -> dict:
    ...
    # Add new DOIs to corpus (metadata not available here, so use summaries)
    merged_corpus = state["paper_corpus"].copy()
    for doi in new_paper_summaries.keys():
        if doi not in merged_corpus:
            merged_corpus[doi] = {"doi": doi}  # Minimal metadata
...
def check_continue(state: Loop2State) -> str:
    ...
    iteration = state["iteration"]
    max_iterations = state["max_iterations"]
    # iteration is 0-indexed, so check if we've completed max_iterations
...
async def run_loop2_standalone(
    ...
    initial_state = Loop2State(
        ...
        iteration=0,  # 0-indexed to match other loops
        max_iterations=max_iterations,
        explored_bases=[],
        is_complete=False,
        decision=None,
