...
class StructuralIssue(BaseModel):
    ...
    severity: Literal["minor", "moderate", "major", "critical"] = Field(description="Impact on document coherence")

    # References use stable IDs from document model
    affected_section_ids: list[str] = Field(description="Section IDs involved in this issue")
    affected_block_ids: list[str] = Field(
        default_factory=list,
        description="Specific block IDs if issue is localized",
    )

    description: str = Field(min_length=20, description="Detailed description of the problem")
    ...
    action_details: str = Field(description="Specific guidance for implementing the fix")
...
class StructuralAnalysis(BaseModel):
    ...
    has_clear_introduction: bool = Field(description="Whether document has a clear introduction")
    has_clear_conclusion: bool = Field(description="Whether document has a clear conclusion")
    narrative_coherence_score: float = Field(ge=0.0, le=1.0, description="Overall narrative flow quality")
    section_organization_score: float = Field(ge=0.0, le=1.0, description="How well sections are organized")

    # Identified issues (ordered by severity)
    issues: list[StructuralIssue] = Field(default_factory=list)

    # Summary
    overall_assessment: str = Field(min_length=50, description="Summary of document structure quality")
...
class SectionMoveEdit(BaseModel):
    ...
    target_position: Literal["before", "after"] = Field(description="Position relative to target")
...
class SectionMergeEdit(BaseModel):
    ...
    merge_strategy: Literal["append", "interleave", "synthesize"] = Field(description="How to combine the sections")
    new_heading: Optional[str] = Field(default=None, description="Optional new heading for merged section")
    justification: str
...
class ContentConsolidationEdit(BaseModel):
    ...
    source_block_ids: list[str] = Field(description="Blocks to consolidate (from different sections)")
    target_section_id: str = Field(description="Where to put consolidated content")
    consolidation_approach: Literal["merge_and_dedupe", "synthesize", "reorganize"] = Field(
        description="How to consolidate the content"
    )
    justification: str
...
class GenerateIntroductionEdit(BaseModel):
    ...
    scope: Literal["document", "section"] = Field(description="Whether for whole document or a section")
    target_section_id: Optional[str] = Field(default=None, description="Section ID (None for document-level)")
    context_section_ids: list[str] = Field(description="Sections to reference for context")
    introduction_requirements: str = Field(description="What the intro should accomplish")
...
class DeleteRedundantEdit(BaseModel):
    ...
    primary_block_id: str = Field(description="Block that adequately covers the content")
    justification: str
...
class EditPlan(BaseModel):
    ...
    execution_order_rationale: str = Field(description="Why edits are ordered this way")
    estimated_word_count_change: int = Field(default=0, description="Estimated net change in word count")
...
class StructureVerification(BaseModel):
    ...
    coherence_score: float = Field(ge=0.0, le=1.0, description="Overall coherence after edits")

    issues_resolved: list[str] = Field(default_factory=list, description="Issue IDs that are now fixed")
    issues_remaining: list[str] = Field(default_factory=list, description="Issues that still need attention")
    regressions: list[str] = Field(default_factory=list, description="New problems introduced by edits")
    ...
    def needs_another_iteration(self) -> bool:
        return self.coherence_score < 0.8 and (len(self.issues_remaining) > 0 or len(self.regressions) > 0)


class CoherenceComparisonResult(BaseModel):
    """Result of comparing two document versions for coherence."""

    preferred_version: Literal["original", "edited"] = Field(description="Which version is more coherent overall")
    original_score: float = Field(ge=0.0, le=1.0, description="Coherence score for original document")
    edited_score: float = Field(ge=0.0, le=1.0, description="Coherence score for edited document")
    confidence: float = Field(ge=0.0, le=1.0, description="Confidence in this assessment")
    reasoning: str = Field(description="Explanation of why one version is preferred")
    key_regressions: list[str] = Field(
        default_factory=list, description="Specific areas where edited version regressed"
    )
...
class SectionEnhancement(BaseModel):
    ...
    enhancement_notes: str = Field(description="Notes about what was enhanced and why")
...
class EnhanceCoherenceReview(BaseModel):
    ...
    coherence_score: float = Field(ge=0.0, le=1.0, description="Overall coherence after enhancements")
    sections_enhanced: list[str] = Field(description="Section IDs that were enhanced in this pass")
    ...
    overall_assessment: str = Field(description="Summary of enhancement pass results")
...
class SectionPolish(BaseModel):
    ...
    polished_content: str = Field(description="The polished section content with improved flow and clarity")
...
class FinalVerification(BaseModel):
    ...
    remaining_issues: list[str] = Field(default_factory=list, description="Any issues that couldn't be resolved")

    overall_assessment: str

    @property
    def overall_score(self) -> float:
        return (self.coherence_score + self.completeness_score + self.flow_score) / 3.0
...
class EditInstruction(BaseModel):
    ...
    merge_source_index: int | None = Field(default=None, description="For merge_into: index of section to merge from")
...
class GlobalAnalysisResult(BaseModel):
    ...
    overall_assessment: str = Field(description="Assessment of document's structural quality (1-2 sentences)")
...
class RewrittenSection(BaseModel):
    ...
    merge_source_index: int | None = Field(default=None, description="If merge: index of merged section")


class V2FinalVerification(BaseModel):
    """Final verification result for the V2 reassembled document."""

    coherence_score: float = Field(ge=0.0, le=1.0, description="Overall coherence (0-1)")
    flow_assessment: str = Field(description="Assessment of document flow")
    issues_found: list[str] = Field(default_factory=list, description="Any remaining issues detected")
    recommendation: Literal["accept", "review", "reject"] = Field(description="Recommendation for the final document")
